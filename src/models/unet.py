...
class UNet(nn.Module):
    ...
    def forward(self, x):
        x1 = self.inc(x)        # 32x16x16
        x2 = self.down1(x1)     # 64x8x8
        x3 = self.down2(x2)     # 128x4x4
        x4 = self.down3(x3)     # 256x2x2
        x5 = self.down4(x4)
        x = self.up1(x5, x4)
        # x = self.up2(x4, x3)    # 128x4x4
        x = self.up2(x, x3)
        x = self.up3(x, x2)     # 64x8x8
        x = self.up4(x, x1)     # 32x16x16
        logits = self.outc(x)   # 3x16x16
        return logits
