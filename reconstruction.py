...
dir_checkpoint = Path('./checkpoints/pretexts')
...
def train_model(
    ...
    model.train()
    epoch_loss = 0
    with tqdm(total=n_train, desc=f'Epoch {epoch}/{100}', unit='img') as pbar:
...
if __name__ == '__main__':
    ...
    val_loader = DataLoader(val_set, shuffle=False, drop_last=True, batch_size=1, num_workers=os.cpu_count(), pin_memory=True)
    ...
    for epoch in range(args.epochs):
        ...
        if epoch%5==0:
            ...
            #     best_score=val_score
                ...
                torch.save(state_dict, str(dir_checkpoint / 'checkpoint_epoch{}.pth'.format(epoch)))
                logging.info(f'Checkpoint {epoch} saved!')
                # logging.info(f'new better score {val_score}')
